...
"""
OKDP-specific patch of the upstream Jupyter Docker Stacks package test.

Adds the Python import mapping for ``jupyter-fs``:

    jupyter-fs -> jupyterfs

The Conda package is named ``jupyter-fs``, while its Python import module is ``jupyterfs``.
